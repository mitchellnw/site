...
In other words, it is possible to train a model that is **small** during inference but **still overparameterized** during training. 
Though we only ever use a small percentage
of the weights during the forwards pass, our network has similar odds at winning the [initialization lottery](https://arxiv.org/pdf/1907.04840v2.pdf)
as a larger network.
