...
In other words, if node $$u$$ can take $$\mathcal{I}_v$$ where the loss _wants_ it to go, we should increase the edge weight from $$u$$ to $$v$$. We therefore modify $$w_{uv}$$ via our update rule

$$w_{uv} \gets w_{uv} + \left\langle \mathcal{Z}_u, g_v \right\rangle$$

where $$\langle \cdot, \cdot \rangle$$ denotes an inner product (these quantities are implicitly treated as vectors).

In practice $$w_{uv}$$ changes only a small amount at each training iteration.
However, if $$\mathcal{Z}_u$$ consistently aligns with $$g_v$$ then $$\lvert w_{uv} \rvert$$ will strengthen to a point where edge $$(u,v)$$ replaces a weaker edge. We show [below](#proofs) that when swapping does occur, it is beneficial under some assumptions.
...
As we show in the [paper](https://arxiv.org/abs/1906.00586), this is equivelantly a [straight-through estimator](https://arxiv.org/abs/1308.3432).
...
The figure below illustrates how this code works. It also emphasizes that we are using a [straight-through estimator](https://arxiv.org/abs/1308.3432) (see paper for more discussion).
...
Here we briefly show that when swapping edges does occur, it is beneficial under some assumptions.
Consider a _Static Neural Graph_ where edge $$(i,k)$$ replaces edge $$(j,k)$$ after the gradient update
for the mini-batch. We may show that when the learning rate $$\alpha$$ is sufficiently small and the node states are fixed then the loss will decrease for the current mini-batch. We skim over some details here
and invite you to reference the [paper](https://arxiv.org/abs/1906.00586), for example we must also
assume that the loss is Lipschitz continuous. The [paper](https://arxiv.org/abs/1906.00586) also
includes a proof of a more general case.

We let $$\tilde w$$ denote the weight $$w$$ after the gradient update. And so by our update rule we let

$$\tilde w_{uv} = w_{uv} + \left\langle \mathcal{Z}_u, g_v \right\rangle$$ where $$g_v = -\alpha {\partial \mathcal{L} \over \partial \mathcal{I}_v}$$. If the learning rate is sufficiently small then
$$\tilde w$$ will be close to $$w$$ and so we assume that $$\tilde w w \gt 0$$. Since edge $$(i,k)$$ replaces edge $$(j,k)$$ we know that $$\lvert w_{ik} \rvert \lt \lvert w_{jk} \rvert $$ and $$ \lvert \tilde w_{ik} \rvert \gt \lvert \tilde w_{jk} \rvert $$ as we choose edges by taking the highest magnitude weights.

Let $$\mathcal{A}$$ be the new input to node $$v$$ if swapping _is_ allowed. Likewise, let $$\mathcal{B}$$ be the new input to node $$v$$ if swapping _is not_ allowed. It suffices
to show that $$\langle \mathcal{A} - \mathcal{I_k}, g_k \rangle \gt \langle \mathcal{B} - \mathcal{I_k}, g_k \rangle$$ as we observe that
$$ \mathcal{L}(\mathcal{\mathcal{I}_k}) - \mathcal{L}(\mathcal{A}) =  \langle \mathcal{A} - \mathcal{I_k}, g_k \rangle + \mathcal{O}(\alpha^2)$$ via a Taylor approximation.

$$\mathcal{A}$$ may be written as $$\tilde w_{ik} \mathcal{Z}_i + \sum_{(u,k) \in \mathcal{E} , \ u\neq i,j} w_{uk} \mathcal{Z}_u$$ and $$\mathcal{B}$$ may be written as $$\tilde w_{jk} \mathcal{Z}_j + \sum_{(u,k) \in \mathcal{E}, \ u\neq i,j} w_{uk} \mathcal{Z}_u$$ and so

$$\langle \mathcal{A} - \mathcal{I_k}, g_k \rangle \gt \langle \mathcal{B} - \mathcal{I_k}, g_k \rangle \iff \tilde w_{ik} \left\langle \mathcal{Z}_i, g_k \right\rangle > \tilde w_{jk} \left\langle \mathcal{Z}_j, g_k \right\rangle$$.

If the magnitude of $$w_{ik}$$ increases while the magnitude of $$w_{jk}$$ decreases then we are done as the left hand side will be positive while the right hand side will be negative. We now examine the case where both magnitudes either increase or increase.

As $$\tilde w_{ik} - w_{ik} = \langle \mathcal{Z}_i, g_k \rangle$$ we are left to show that $$\tilde w_{ik}^2 - w_{ik}\tilde w_{ik} > \tilde w_{jk}^2 - w_{jk}\tilde w_{jk}$$. Moreover, since  $$\tilde w w \gt 0$$ we may simplify to $$ \lvert \tilde w_{ik}\rvert^2 - \lvert w_{ik} \rvert \lvert \tilde w_{ik} \rvert > \lvert \tilde w_{jk} \rvert ^2 - \lvert w_{jk} \rvert \lvert \tilde w_{jk} \rvert$$.

Rearranging we find that
$$ \lvert w_{jk} \rvert \lvert \tilde w_{jk}\rvert  - \lvert w_{ik} \rvert \lvert \tilde w_{ik} \rvert > \lvert \tilde w_{jk} \rvert ^2 -  \lvert \tilde w_{ik}\rvert^2$$ where the right hand side is negative by assumption. Moreover, when the learning rate is small enough the left hand side is positive. If $$\epsilon = \lvert w_{jk} \rvert - \lvert w_{ik} \rvert $$. In the extreme case where $$\lvert \tilde w_{ik} \rvert$$ _just exceeds_ $$\lvert \tilde w_{jk} \rvert$$ then

$$ \lvert w_{jk} \rvert \lvert \tilde w_{jk}\rvert  - \lvert w_{ik} \rvert \lvert \tilde w_{ik} \rvert \approx \epsilon  \lvert \tilde w_{jk} \rvert > 0$$.
