...
$$\mathcal{A}$$ may be written as $$\tilde w_{ik} \mathcal{Z}_i + \sum_{(u,k) \in \mathcal{E} \setminus \{i, j\}} w_{uk} \mathcal{Z}_u$$ and $$\mathcal{B}$$ may be written as $$\tilde w_{jk} \mathcal{Z}_j + \sum_{(u,k) \in \mathcal{E} \setminus \{i, j\}} w_{uk} \mathcal{Z}_u$$ and so
...
$$ \lvert w_{jk} \rvert \lvert \tilde w_{jk}\rvert  - \lvert w_{ik} \rvert \lvert \tilde w_{ik} \rvert > \lvert \tilde w_{jk} \rvert ^2 -  \lvert \tilde w_{ik}\rvert^2$$ where the right hand side is negative by assumption. Moreover, when the learning rate is small enough the left hand side is positive. If $$\epsilon = \lvert w_{jk} \rvert - \lvert w_{ik} \rvert $$. In the extreme case where $$\lvert \tilde w_{ik} \rvert$$ just exceeds $$\lvert \tilde w_{jk} \rvert$$ then
